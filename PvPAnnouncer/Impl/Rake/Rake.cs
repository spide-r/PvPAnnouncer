...
public class Rake
{
        ...
        double minKeywordFrequency = 1)
    {
        ...
        _stopWords = StopListHelper.ParseFromLanguage(languageAbbrev);
    }
    ...
    private Dictionary<string, double> CalculateWordScores(IEnumerable<string> lowerCasedPhraseList)
    {
        ...
        foreach (var item in wordFrequency) wordDegree[item.Key] = wordDegree[item.Key] + wordFrequency[item.Key];
    ...
    private static readonly Regex splitter = new(@"[^a-zA-Z0-9_\+\-/]", RegexOptions.Compiled);

    /// <summary>
    ///     Utility function to return a list of all words that are have a length greater than a specified number of
    ///     characters.
    /// </summary>
    ...
    private static bool IsAcceptable(string phrase, int minCharLength, int maxWordsLength)
    {
        ...
        while (wordSplitter.TryGetNext(out _)) wordCount++;

        if (wordCount > maxWordsLength) return false;
